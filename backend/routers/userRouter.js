...
router.post('/register', async (req, res, next) => {
  ...
  try {
    ...
    if (uExists) return next([400, { error: 'username', message: 'Username is used by someone else' }]);

    const eExists = await User.findByEmail(userData.email);
    if (eExists) return next([400, { error: 'email', message: 'Email is used by someone else' }]);
...
router.post('/login', async (req, res, next) => {
  ...
  if (password.length < 6) return next([400, 'Password must be at least 6 characters.']);
